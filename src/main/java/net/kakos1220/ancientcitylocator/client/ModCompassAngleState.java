...
public class ModCompassAngleState extends ModNeedleDirectionHelper {
    public static final MapCodec<ModCompassAngleState> MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(Codec.BOOL.optionalFieldOf("wobble", true).forGetter(ModNeedleDirectionHelper::wobble), ModCompassAngleState.ModCompassTarget.CODEC.fieldOf("target").forGetter(ModCompassAngleState::target)).apply(i, ModCompassAngleState::new));
    private final ModNeedleDirectionHelper.Wobbler wobbler = this.newWobbler(0.8F);
    private final ModNeedleDirectionHelper.Wobbler noTargetWobbler = this.newWobbler(0.8F);
    private final ModCompassAngleState.ModCompassTarget compassTarget;
    private final RandomSource random = RandomSource.create();

    public ModCompassAngleState(final boolean wobble, final ModCompassAngleState.ModCompassTarget compassTarget) {
        super(wobble);
    ...
    protected ModCompassAngleState.ModCompassTarget target() {
        return this.compassTarget;
    }

    @Environment(EnvType.CLIENT)
    public static enum ModCompassTarget implements StringRepresentable {
        ANCIENT_CITY("ancient_city") {
            public GlobalPos get(final ClientLevel level, final ItemStack itemStack, final @Nullable ItemOwner owner) {
                BlockPos pos = itemStack.get(ModDataComponentTypes.TARGET);
        ...
        public static final Codec<ModCompassAngleState.ModCompassTarget> CODEC = StringRepresentable.fromEnum(ModCompassAngleState.ModCompassTarget::values);
        private final String name;

        private ModCompassTarget(final String name) {
            this.name = name;
        }
